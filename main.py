...
contrast = 70                              # 对比度，会影响到检测，阴影和圆圈痕迹都会更重

###################################################################################

# 初始化串口协议
serial_protocol = SerialProtocol(port='/dev/ttyS0', baudrate=9600)
                ...
                if minH > rect_min_limit and minW > rect_min_limit:
                    ...
                    if circle_dist > 0:
                        ...
                        if (not find_circle) or (center_new):
                            ...
                            # 打印矩形中心点坐标
                            print(f"矩形中心点坐标: x={original_center_point[0]}, y={original_center_point[1]}")
                            
                            # 发送中心点坐标到串口
                            try:
                                serial_protocol.send_rect_center(original_center_point[0], original_center_point[1])
